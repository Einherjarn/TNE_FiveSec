  ...
  function FiveSec:Start()
    ...
    frame.delay = TNEUtils.Select(delay < 0,  delay, delay) -- 2 - delay here
    --frame.delay = delay
    --pxd(frame.delay)
  ...
  function FiveSec:Stop()
	...
	if (FiveSec.lastGain) then
      delay = FiveSec.lastGain - curr -- not 100% accurate
    end
    ...
    spark:Hide()
    --text:SetText(format("Regenerating")) -- text:SetText(format(TNE_FiveSec_REGENERATION_IN_INT, 0))
  ...
  function FiveSecBarFrame_OnEvent()
    ...
    if (event == "UNIT_MANA" or event == "UNIT_MAXMANA") then
	  ...
	  local test = GetTime() - FiveSec.lastGain
      if (mana > FiveSec.mana and FiveSec.mana ~= 0 and test >= 1.995) then --imperfect fix for black grasp triggering mana updates
        FiveSec.lastGain = GetTime()
		--text:SetText("DEBUGLOL");
